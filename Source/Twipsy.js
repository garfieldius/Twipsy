			...
			if (!stylesAreInjected) {
				...
				var stylesText =
					'.twipsy { display: block; position: absolute; visibility: visible; padding: 5px; font-size: 11px; z-index: 1000; filter: alpha(opacity=80); -khtml-opacity: 0.8; -moz-opacity: 0.8; opacity: 0.8;}\
					 .twipsy-fade { -webkit-transition: opacity 0.15s linear;-moz-transition: opacity 0.15s linear;-ms-transition: opacity 0.15s linear;-o-transition: opacity 0.15s linear;transition: opacity 0.15s linear;opacity: 0;} \
					 .twipsy.twipsy-fade.twipsy-in { filter: alpha(opacity=80); -khtml-opacity: 0.8; -moz-opacity: 0.8; opacity: 0.8;}\
	...
	var Twipsy = new Class({
		...
		show: function() {
			var pos, actualWidth, actualHeight, placement, twipsyElement, position,
				offset, size, twipsySize;

			if (this.hasContent() && this.enabled) {
				twipsyElement = this.setContent().getTip();

				if (this.options.animate) {
					twipsyElement.addClass('twipsy-fade');
				}
		...
		hide: function() {
			...
			tip.removeClass('twipsy-in');

			Browser.Features.transition && tip.hasClass('twipsy-fade')
				? tip.addEvent(transitionEndEventName, removeElement)
				: removeElement();
